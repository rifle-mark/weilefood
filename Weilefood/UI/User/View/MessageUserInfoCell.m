...
@implementation MessageUserInfoCell
...
+ (NSString *)reuseIdentify {
    return @"MessageUserInfoCellIdentify";
}
...
- (void)updateConstraints {
    ...
    if (self.message) {
        ...
        [self.messageBgV mas_remakeConstraints:^(MASConstraintMaker *make) {
            ...
            make.left.equalTo(self.avatarV.mas_right);
            make.bottom.equalTo(self.messageBgV.superview);
            make.right.equalTo(self.messageBgV.superview).with.offset(-45);
        }];
    }
...
- (UIImageView *)messageBgV {
    if (!_messageBgV) {
        ...
        _messageBgV.image = [UIImage imageNamed:@"message_user"];
